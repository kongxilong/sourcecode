...
bool CUiFeatureTestWindow::ShowUiFeatureTestWindow(DWORD dwMainThreadId)
{
	...
	// 取得皮肤包的路径
	CStringW strSkinPath = PathHelperW(_T("..\\SkinPackage\\004_ICtrlIEWebBrowser\\004_ICtrlIEWebBrowser.ufd"));

	CRect WndRct(0, 0, 0, 0);
	// 根据皮肤包创建窗口，如果窗口大小为0，则会根据皮肤包中的大小自动设置窗口的大小。
	// 第一个参数为皮肤包密码
	return CreateFeatureWindow(NULL, strKernelDllPath, strSkinPath, L"TestWindow", NULL, WndRct, SW_SHOW, WS_OVERLAPPED | WS_THICKFRAME | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, NULL);
}
...
{
	m_pIeCtrl = GET_CONTROL(ICtrlIEWebBrowser, "TestIeCtrl");
	if (m_pIeCtrl == NULL)
	{
		assert(false);
		return;
	...
	m_pIeCtrl->OpenUrl(m_pIeCtrl->GetUrl());
}

...
class CUiFeatureHtmlWindow : public CWindowImpl<CUiFeatureHtmlWindow, CAxWindow>
{
public:
	DECLARE_WND_SUPERCLASS(_T("UiFeatureHtmlView"), CAxWindow::GetWndClassName())

	CUiFeatureHtmlWindow()
		: m_dwDocHostFlags(0)
	...
	BEGIN_MSG_MAP(CUiFeatureHtmlWindow)
		...
		DOCHOSTUIFLAG_THEME | DOCHOSTUIFLAG_FLAT_SCROLLBAR, BOOL bAllowContextMenu = FALSE)
	{
		...
		return CWindowImpl<CUiFeatureHtmlWindow, CAxWindow>::Create(hWndParent, rect, lpURL, dwStyle, dwExStyle, MenuOrID, lpCreateParam);
	}
		...
		DOCHOSTUIFLAG_THEME | DOCHOSTUIFLAG_FLAT_SCROLLBAR, BOOL bAllowContextMenu = FALSE)
	{
		...
		m_spExternal		= pExternalDispatch;
		m_dwDocHostFlags	= dwDocHostFlags;
		HWND hWnd	= CWindowImpl<CUiFeatureHtmlWindow, CAxWindow>::Create(hWndParent, rect, L"mshtml:<html></html>", dwStyle, dwExStyle, MenuOrID, lpCreateParam);
...
class CUiFeatureHtmlView : public CUiFeatureHtmlWindow,
	public IDispEventSimpleImpl<0, CUiFeatureHtmlView, &DIID_DWebBrowserEvents2>
{
public:
	...
	{
		HWND hWnd = CUiFeatureHtmlWindow::Create(pExternalDispatch, lpURL, hWndParent, rect, dwStyle, dwExStyle, MenuOrID, lpCreateParam, dwDocHostFlags);
		if (hWnd)
		{
			CComPtr<IWebBrowser2> spWebBrowser2;
			HRESULT hRet = QueryControl(IID_IWebBrowser2, (void**)&spWebBrowser2);
	...
	BEGIN_MSG_MAP(CUiFeatureHtmlView)
		MESSAGE_HANDLER(WM_CREATE, OnCreate)
		CHAIN_MSG_MAP(CUiFeatureHtmlWindow)
		MESSAGE_HANDLER(WM_DESTROY, OnDestroy)
	END_MSG_MAP()

	BEGIN_SINK_MAP(CUiFeatureHtmlView)
	...
	LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		LRESULT lRet = CUiFeatureHtmlWindow::OnCreate(uMsg, wParam, lParam, bHandled);
		// Connect events
		CComPtr<IWebBrowser2> spWebBrowser2;
		HRESULT hRet = QueryControl(IID_IWebBrowser2, (void**)&spWebBrowser2);
		if(SUCCEEDED(hRet))
		{
			HRESULT hr = IDispEventSimpleImpl<0, CUiFeatureHtmlView, &DIID_DWebBrowserEvents2>::DispEventAdvise(spWebBrowser2, &DIID_DWebBrowserEvents2);
	...
	LRESULT OnDestroy(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		HRESULT hRet = QueryControl(IID_IWebBrowser2, (void**)&spWebBrowser2);
		if(SUCCEEDED(hRet))
			IDispEventSimpleImpl<0, CUiFeatureHtmlView, &DIID_DWebBrowserEvents2>::DispEventUnadvise(spWebBrowser2, &DIID_DWebBrowserEvents2);
...
__declspec(selectany) _ATL_FUNC_INFO CUiFeatureHtmlView::NewWindow3_Info = { CC_STDCALL, VT_EMPTY, 5, {VT_BYREF | VT_DISPATCH, VT_BYREF | VT_BOOL, VT_UINT, VT_BSTR, VT_BSTR}};

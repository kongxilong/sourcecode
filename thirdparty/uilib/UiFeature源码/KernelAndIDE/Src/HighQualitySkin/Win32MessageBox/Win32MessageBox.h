...
// 当不是采用模态的方式弹出 CWin32MessageBox 的时候，如果需要 CWin32MessageBox 的返回值，那需要继承这个接口进行回调
...
class CWin32MessageBox : public CUiFeatureWindow
{
public:
	CWin32MessageBox();
	virtual ~CWin32MessageBox();
...
protected:
	...
	void EndMessageBox(DWORD nReturn, BOOL bSendClose = TRUE);

private:
	HWND m_hParent;
	HICON m_DlgIcon;
	INT m_nDlgIconId;
	BOOL m_bIsParentEnabled;

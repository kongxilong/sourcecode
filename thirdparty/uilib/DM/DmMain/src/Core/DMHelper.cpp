...
namespace DM
{
	bool GetRootPathW(wchar_t *pszPath, DWORD dwSize)
	{
		...
		do 
		{
			...
			{
				LOG_ERR(L"[mid]GetModuleFileNameW fail\n");
				break;
			}

			if (wcslen(szPath)>=dwSize)
			{
				LOG_ERR(L"[mid]dwSize too small\n");
				break;
			}

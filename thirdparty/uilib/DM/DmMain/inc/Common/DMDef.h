...
#define  LOG_ERR(fmt, ...)							 DMLogDispatch::LogW(DMLOG_ERR,__FUNCTIONW__,__FILEW__, __LINE__,fmt,__VA_ARGS__)
#else
#define  LOG_INFO(fmt, ...) 
#define  LOG_ERR(fmt, ...)
#endif//DMLOG_ON
// LOG_USER方便在VS调试下定位问题使用，使用完必须清除!!
#define  LOG_USER(fmt, ...)							 DMLogDispatch::LogW(DMLOG_INFO,__FUNCTIONW__,__FILEW__, __LINE__,fmt,__VA_ARGS__)

...
IMPLEMENT_DYNAMIC(CEmpEdit, CDialogEx)

CEmpEdit::CEmpEdit(CWnd* pParent /*=NULL*/)
	: CDialogEx(CEmpEdit::IDD, pParent)
	, m_addr_emp(_T(""))
	, m_name_emp(_T(""))
	, m_num_emp(_T(""))
	, m_phone_emp(_T(""))
...
CEmpEdit::CEmpEdit(CEmpInfoView* p_dlg,CWnd* pParent /*=NULL*/)
	: CDialogEx(CEmpEdit::IDD, pParent)
{
	m_pEmpInfoView = p_dlg;
}
CEmpEdit::~CEmpEdit()
{
	m_pEmpInfoView = NULL;
...
void CEmpEdit::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_POSTION, m_pos_emp);
	DDX_Control(pDX, IDC_STATIC_TIP, m_tip);
}


BEGIN_MESSAGE_MAP(CEmpEdit, CDialogEx)
	ON_WM_CTLCOLOR()
	ON_WM_TIMER()
	ON_MESSAGE(WM_STARTTASK_EmpEdit, &CEmpEdit::OnStartTask)
	ON_MESSAGE(WM_ENDTASK_EmpEdit, &CEmpEdit::OnEndTask)
	ON_BN_CLICKED(IDC_BTN_SAVE, &CEmpEdit::OnBnClickedBtnSave)
	ON_BN_CLICKED(IDC_BTN_CANCEL, &CEmpEdit::OnBnClickedBtnCancel)
	ON_WM_CLOSE()
END_MESSAGE_MAP()


// CEmpEdit 消息处理程序
BOOL   CEmpEdit::OnInitDialog()
{
...
HBRUSH CEmpEdit::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CEmpEdit::OnTimer(UINT_PTR nIDEvent)
{
...
afx_msg LRESULT CEmpEdit::OnStartTask(WPARAM wParam, LPARAM lParam)
{
...
afx_msg LRESULT CEmpEdit::OnEndTask(WPARAM wParam, LPARAM lParam)
{
	KillTimer(1);
	m_time_take = 0;
...
void CEmpEdit::OnBnClickedBtnSave()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData(TRUE);
	...
	CSock::StartReqEmpEdit(this);
}


void CEmpEdit::OnBnClickedBtnCancel()
{
	// TODO: 在此添加控件通知处理程序代码
	CDialogEx::OnCancel();
}
...
void CEmpEdit::OnClose()
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	CSock::m_pEmpEdit = NULL;
	CDialogEx::OnClose();
}


BOOL CEmpEdit::IsItemValidity(CString _source,CString _item_name,BOOL bCheckEmpty,BOOL bCheckNum,BOOL bCheckDateFormat,int str_len_max,int data_min_len,BOOL bCheckDot)
{
...
BOOL CEmpEdit::ContainsCharsRemain(CString str)     //检查字符串是否包含子字符串
{
	CString temp1 = _T("[#");
	CString temp2 = _T("#]");
	CString temp3 = _T("{#");
...
BOOL CEmpEdit::IsNum(CString str,BOOL bCheckDot)
{
	WT;
	char *p = T2A(str);
	int i = 0;
